...
export const useGamesData = ({ futureOnly = false }: UseGamesDataProps = {}) => {
  ...
  useEffect(() => {
    const fetchGames = async () => {
      if (!isAuthenticated) {
        setError("Authentication required to view games data");
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          for (const eventId in groupedByEvent) {
            ...
            const team1Info = h2hOdds.find((odd: OddsRow) => odd.name === t1name);
            const team2Info = h2hOdds.find((odd: OddsRow) => odd.name === t2name);
            
            if (!team1Info || !team2Info) continue; // Skip if we can't identify the teams
            
            // Find spread info
            const team1Spread = spreadOdds.find((odd: OddsRow) => odd.name === t1name);
            const team2Spread = spreadOdds.find((odd: OddsRow) => odd.name === t2name);
            
            // Find total info - first look for over/under names, then fall back to any total odds
            let overTotal = totalOdds.find((odd: OddsRow) => odd.name.toLowerCase() === 'over');
            let underTotal = totalOdds.find((odd: OddsRow) => odd.name.toLowerCase() === 'under');
  ...
  }, [futureOnly, supabase, isAuthenticated]);
